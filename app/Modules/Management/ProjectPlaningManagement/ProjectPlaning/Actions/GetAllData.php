...
namespace App\Modules\Management\ProjectPlaningManagement\ProjectPlaning\Actions;
        ...
        try {
            ...
            if (request()->has('search') && request()->input('search')) {
                ...
                $data = $data->where(function ($q) use ($searchKey) {
                    $q->where('planning_steps', 'like', '%' . $searchKey . '%');

                    $q->orWhere('statistics', 'like', '%' . $searchKey . '%');

                });
            }

            if (request()->has('latest_data') && request()->input('latest_data')) {
                $data = $data->active()->latest()->first();
            ...
            if (request()->has('get_all') && (int) request()->input('get_all') === 1) {
                $data = $data
                    ...
                    ->get();
                return entityResponse($data);
            } else if (request()->has('latest_data') && request()->input('latest_data')) {
